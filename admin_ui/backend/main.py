...
from api import config, system, wizard, logs, local_ai, ollama, mcp, calls, outbound, tools, docs, users, billing  # noqa: E402
...
app.include_router(users.router, prefix="/api/users", tags=["users"], dependencies=[Depends(auth.get_current_user)])
app.include_router(billing.router, prefix="/api/billing", tags=["billing"], dependencies=[Depends(auth.get_current_user)])
